#' Multi-trait STAAR (MultiSTAAR_sp) procedure using omnibus test (dgCMatrix version)
#'
...
#' @return \code{results_STAAR_O}: the multi-trait STAAR-O (MultiSTAAR-O) p-value
...
#' @return \code{results_ACAT_O}: the multi-trait ACAT-O p-value that aggregated multi-trait SKAT(1,25),
#' SKAT(1,1), Burden(1,25), Burden(1,1), ACAT-V(1,25), and ACAT-V(1,1) using Cauchy method.
#' @return \code{results_STAAR_S_1_25}: a vector of multi-trait STAAR-S(1,25) p-values,
...
#' @return \code{results_STAAR_S_1_1}: a vector of multi-trait STAAR-S(1,1) p-values,
...
#' p-value by aggregating these p-values using Cauchy method.
#' @return \code{results_STAAR_B_1_25}: a vector of multi-trait STAAR-B(1,25) p-values,
...
#' p-value by aggregating these p-values using Cauchy method.
#' @return \code{results_STAAR_B_1_1}: a vector of multi-trait STAAR-B(1,1) p-values,
...
#' p-value by aggregating these p-values using Cauchy method.
#' @return \code{results_STAAR_A_1_25}: a vector of multi-trait STAAR-A(1,25) p-values,
...
#' p-value by aggregating these p-values using Cauchy method.
#' @return \code{results_STAAR_A_1_1}: a vector of multi-trait STAAR-A(1,1) p-values,
...
MultiSTAAR_sp <- function(genotype_sp,MAF=NULL,obj_nullmodel,annotation_phred=NULL,
  ...
  if(sum(RV_label) >= rv_num_cutoff){
    ...
    if(!obj_nullmodel$sparse_kins){
      ...
      pvalues <- MultiSTAAR_O_SMMAT(G,P,residuals.phenotype,
      ...
      pvalues <- MultiSTAAR_O_SMMAT_sparse(G,Sigma_i,Sigma_iX,cov,residuals.phenotype,
                                           weights_B=w_B,weights_S=w_S,weights_A=w_A,
                                           mac=as.integer(round(MAF*2*dim(G)[1]/obj_nullmodel$n.pheno)),
                                           n_pheno=obj_nullmodel$n.pheno)
    }
